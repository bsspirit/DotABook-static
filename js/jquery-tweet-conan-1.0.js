$(document).ready(function(){
	...
	function ajax_msg(page){
		$.getJSON('/heroes/msgs/?p='+page,function(res){
			$('#tweets').html(msg_html(res));
			render_btn();
			render_dialog();
			if($('#tweets').attr('login')=='True'){
				render_namecard('#tweets')
			}
			render_page();
		});
	}
	...
	function render_btn(){
		$('#tweets').find('.b-msg-btn').each(function(){
	...
	function msg_html(res){
		...
		for (var i=0;i<msgs.length;i++){
			...
			html += '<p>英雄：<a href="/heroes/'+msgs[i].hid+'"><span class="info3">'+msgs[i].hnamecn+'</span></a>&nbsp;&nbsp;&nbsp;&nbsp;'
			if(msgs[i].grade){html += 'Gank:'+msgs[i]['grade.gank']+'&nbsp;Push:'+msgs[i]['grade.push']+'&nbsp;DPS:'+msgs[i]['grade.dps']+'&nbsp;辅助:'+msgs[i]['grade.assist']+'&nbsp;肉盾:'+msgs[i]['grade.defend']}
			html += '</p>'
			html += '</div>'
			...
			html += '<a href="/heroes/'+msgs[i].hid+'" class="btn4">打分</a>' 
			html += '</div>'
			html += '<div class="clear"></div>'
			html += '</li>'
		}
